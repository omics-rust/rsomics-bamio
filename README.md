...
Current consumers include `rsomics-bam`, `rsomics-call`, `rsomics-pileup`, and
`rsomics-methyl`; the same policy-free record contract is planned for
`rsomics-minimap2`.
...
policy. `decoded_cigar` validates ordinary BAM operations and expands the
standard `CG:B,I` representation when a record exceeds BAM's operation-count
limit.
